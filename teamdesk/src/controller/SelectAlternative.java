...
public class SelectAlternative implements Initializable {
    ...
    public static ObservableList<Component> listaAlternativas = FXCollections.observableArrayList();
    ...
    public void insert_components_operation() {
        if (table_view_components.getSelectionModel().getSelectedItem() == null) {
            ...
            AlterarOperacao.operacaoPublica.getComponents().get(AlterarOperacao.indicePublico).setAlternative(alternativa);
            listaAlternativas.set(AlterarOperacao.indicePublico,alternativa);
            AlterarOperacao.observableListSelectComps.remove(alternativa);
            btn_cancel_component.getScene().getWindow().hide();
        }
    }
